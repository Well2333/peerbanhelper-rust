...
1. **BTN**：legacy(<20) 分支、PoW 自动获取挑战(当前 ability 多 pow=false)、reconfigure/ip_query 调度未处理(非必需)。
2. **GeoIP 数据**：放 `<data>/geoip/GeoLite2-City.mmdb` + `GeoLite2-ASN.mmdb` 即启用;GeoCN（net-type/行政区划）未移植。
3. **下载器**：增量封禁/EE shadowban 真机补验。
4. **WebUI**：订阅的增删改 UI（当前经 YAML 编辑 + 状态展示）、图表、地理展示美化。
5. **工程**：channel 并行流水线 + WatchDog + 每小时 banlist 快照（当前顺序够用）；配置注释保留迁移（R4）；LogBuffer broadcast 真推送（当前 WS 700ms 轮询）。
6. **axum 版本**：当前 0.7（路由用 `:param`）;若升级到 0.8 需把 `:param` 改回 `{param}`。
